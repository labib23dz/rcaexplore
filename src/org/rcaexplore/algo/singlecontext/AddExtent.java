 ...
 * one by one.*/
public class AddExtent extends SingleContextAlgorithm<OptimizedConceptOrder> {
	
	private OptimizedConceptLattice lattice;
	
	public AddExtent(ObjectAttributeContext context) {
		super(context);
	}
	...
	private OptimizedConcept addExtent(BitVector extent,OptimizedConcept generatorParam, OptimizedConceptLattice lattice) {		
		OptimizedConcept generator = getSmallestContainingConcept(extent, generatorParam);
		
		/** if the generator has exactly the same extent*/
	...
	private OptimizedConcept getSmallestContainingConcept(BitVector extent, OptimizedConcept generator) {
		...
		while ( isMaximal ) {
			isMaximal = false;
			BitVector inter=new BitVector(context.getEntityNb());
