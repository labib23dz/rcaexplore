...
public class CLFAnalyzer2 {
	...
	public String makeImplicationRulesTraitImpact( String traitToConsider){
														...
														if (!pcConcept.getExtent().isEmpty())
															...
															for (Attribute pcAtt : pcConcept.getIntent())
															{
																if (pcAtt instanceof BinaryAttribute){
																	...
																	fw.append("["+((BinaryAttribute)pcAtt).getValue()
																		+"] at level ["
	...
	public String makeImplicationRulesPCImpact( String pcToConsider){
												...
												for (Attribute taxonsAtt : taxonConcept.getIntent())
												{
													if (taxonsAtt instanceof GenericRelationalAttribute 
														...
														if(!traitConcept.getExtent().isEmpty()) {
															...
															for (Attribute traitAtt: traitConcept.getIntent()) {
																fw.append("["+((BinaryAttribute)traitAtt).getValue()
																		+"] at level ["
	...
	public void makeAllImplicationRulesPCImpact(String file) throws IOException{
		...
		for (Attribute att : co.getAttributes())
		{
			if (att instanceof BinaryAttribute){
				fw.append(makeImplicationRulesPCImpact(((BinaryAttribute) att).getValue()));
			}
		}
		fw.close();
	...
	public void makeAllImplicationRulesTraitImpact(String file) throws IOException{
		...
		{
			if (att instanceof BinaryAttribute){
				fw.append(makeImplicationRulesTraitImpact(((BinaryAttribute) att).getValue()));
			}
		}
	...
	public static void main(String[] args) {
		CLFAnalyzer2 analyzer=new CLFAnalyzer2();

		analyzer.loadCLF("data/rlq-20130430/result.xml");
		//analyzer.analyze();
		try {
			analyzer.makeAllImplicationRulesPCImpact("data/rlq-20130430/pcrules.txt");
			analyzer.makeAllImplicationRulesTraitImpact("data/rlq-20130430/rules.txt");
		} catch (IOException e) {
			e.printStackTrace();
		}
