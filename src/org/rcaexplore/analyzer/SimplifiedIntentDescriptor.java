...
public class SimplifiedIntentDescriptor implements ConceptDescriptor {
	...
	public void describeConcept(ConceptDescription conceptDescription) {
		ArrayList<Attribute> att=conceptDescription.getInitialConcept().getSimplifiedIntent();
		...
		for (Attribute a : att)
		{
			if (a instanceof GenericRelationalAttribute)
				...
				conceptDescription.getFormalAttributes().add(new FormalAttributeDescription(((BinaryAttribute) a).getValue(),conceptDescription.getInitialConcept().getConceptOrder().getName()));
		}
